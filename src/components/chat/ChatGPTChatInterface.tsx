...
export interface ChatGPTChatInterfaceProps {
...
export const ChatGPTChatInterface = forwardRef<ChatInterfaceHandle, ChatGPTChatInterfaceProps>(function ChatGPTChatInterface({
  conversationId,
  onNewConversation
}: ChatGPTChatInterfaceProps, ref) {
  ...
  useImperativeHandle(ref, () => ({
    exportConversation: () => {
      try {
        const data = {
          ...
          messages: messages.map(m => ({ id: m.id, role: m.role, content: m.content, created_at: (m as { created_at?: string }).created_at ?? null }))
        }
        const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' })
        const url = URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = `oceochat-conversation-${conversationId || 'new'}-${Date.now()}.json`
    ...
    exportMarkdown: () => {
      try {
        const md = messages
          .map(m => `### ${m.role === 'user' ? 'You' : 'OceoChat'}\n\n${m.content}\n`)
          .join('\n')
        const blob = new Blob([md], { type: 'text/markdown' })
        const url = URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = `oceochat-conversation-${conversationId || 'new'}-${Date.now()}.md`
        document.body.appendChild(a)
        a.click()
        a.remove()
        URL.revokeObjectURL(url)
    ...
    copyConversation: async () => {
      try {
        const text = messages
          .map(m => `${m.role === 'user' ? 'You' : 'OceoChat'}: ${m.content}`)
          .join('\n\n')
  ...
  const handleSend = async (value: string, _imageFile?: File) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#343541] relative">
      ...
      <div className="flex-1 overflow-hidden relative">
        <div className="h-full overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent hover:scrollbar-thumb-gray-600">
            ...
            <div className="h-full">
              {messages.map((message, index) => (
                <div
                  ...
                  className={cn("w-full py-3 px-3 sm:px-4 bg-[#343541]")}
                >
                  <div className="max-w-3xl mx-auto">
                    <div className="flex gap-4 sm:gap-6">
                      ...
                      {/* Message Content - ChatGPT Bubble Style */}
                      <div className={cn(
                        "flex-1 min-w-0",
                        message.role === 'user' ? 'flex justify-end' : 'flex justify-start'
                      )}>
                        <div className={cn(
                          "rounded-lg px-4 py-3 text-[15px] leading-7 whitespace-pre-wrap break-words max-w-2xl",
                          message.role === 'user'
                          ...
                          {/* Charts and References for assistant messages */}
                          {message.role === 'assistant' && (
                            <>
                              <MessageCharts 
                                charts={message.metadata?.charts as any[]} 
                                oceanData={message.metadata?.oceanData as Record<string, unknown>}
                              />
                              <MessageReferences 
                                references={message.metadata?.research?.references as any[]}
                              />
                            </>
                          )}
                        </div>
                          ...
                          <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 mt-2">
                            <div className="flex items-center gap-1">
                              <Button
                                variant="ghost"
                                size="sm"
                                className="h-8 w-8 p-0 text-gray-300 hover:text-white hover:bg-[#2a2b32] rounded-lg transition-colors"
                            ...
                            <div className="mt-2">
                              <ExportActions
                                ...
                                charts={(message.metadata?.charts as unknown[]) || []}
              ...
              {isLoading && (
                <div className="w-full py-6 px-3 sm:px-4 bg-[#343541]">
                  <div className="max-w-3xl mx-auto">
                    <div className="flex gap-4 sm:gap-6">
                      <Avatar className="w-8 h-8 bg-gradient-to-br from-green-500 to-emerald-500">
                        <AvatarFallback className="bg-transparent">
                      ...
                      <div className="flex-1">
                        <div className="bg-[#444654] rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3 max-w-xs">
      ...
      {/* Modern Input Area */}
      <div className="border-t border-[#2a2b32] bg-[#343541] sticky bottom-0">
        <div className="max-w-3xl mx-auto">
          <PromptBox
            onSubmit={handleSend}
            className="px-3 sm:px-4 py-4 sm:py-6 bg-transparent"
          />
